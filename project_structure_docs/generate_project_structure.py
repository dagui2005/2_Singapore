...
class ProjectStructureAnalyzer:
    ...
    def load_file_descriptions(self):
        ...
        return {
            # 根目录文件
            "README.md": "项目主文档，包含完整的社会议工具使用说明",
            ...
            "lta_dynamic_data_downloader.py": "⭐ 主程序（完整集成版）- LTA数据下载核心工具",
    ...
    def generate_markdown(self, structure, output_path):
        """生成Markdown格式的项目结构文档"""
        md_content = f"""# 新加坡交通数据项目 - 文件结构说明文档

**生成时间**: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}  
**项目路径**: {self.project_root}  
**分析工具**: generate_project_structure.py
...
"""
        ...
        # 生成详细文件说明
        md_content += self._generate_detailed_descriptions(self.structure)
...
4. **代码复用** - 将通用功能提取到 `project_structure_docs/scripts/` 中
    ...
    def _generate_tree_markdown(self, dir_path, structure, prefix=""):
        ...
        for i, (name, info) in enumerate(items):
            ...
            if info.get("type") == "directory":
                ...
                content += f"{prefix}{current_prefix}{name} ({size_kb:.1f} KB)\n"

        return content

    def _generate_detailed_descriptions(self, structure, level=0):
        """生成详细的文件说明"""
        content = ""
        indent = "  " * level
        ...
        for name, info in structure.get("children", {}).items():
            if info.get("type") == "directory":
                content += f"\n{indent}### {name}/\n\n"
                content += self._generate_detailed_descriptions(info, level + 1)
            else:
                desc = info.get("description", "待补充说明")
                content += f"{indent}- **{name}**: {desc}\n"
...
"""
    ...
    def _collect_statistics(self, structure, stats):
        ...
        if structure.get("type") == "directory":
            ...
            stats["total_files"] += 1
            stats["total_size_mb"] += structure.get("size", 0) / (1024 * 1024)

            # 统计文件类型
            ext = os.path.splitext(structure.get("name", ""))[1].lower() or "(无扩展名)"
...
def main():
    ...
    analyzer.generate_markdown(analyzer.structure, output_path)

    print(f"[SUCCESS] 完成！文档已保存到: {output_path}")
